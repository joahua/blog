...
<br />
<h2><?php _e('WordPress Releases'); ?></h2>
<h3>Recent Releases</h3>
<?php
$rss = @fetch_rss('http://wordpress.org/development/category/releases/rss');
if ( isset($rss->items) && 0 != count($rss->items) ) {
$rss->items = array_slice($rss->items, 0, 3);
foreach ($rss->items as $item ) {
?>
<ul>
<li><a href='<?php echo wp_filter_kses($item['link']); ?>'>
<?php echo wp_specialchars($item['title']); ?></a></li>
</ul>
<?php
	}
}
